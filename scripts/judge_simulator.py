...
class OpenAIProvider(LLMProvider):
    ...
    def complete(self, prompt: str, system: str = None) -> str:
        ...
        req = urlrequest.Request(
            ...
            data=body,
            headers={"Authorization": f"Bearer {self.api_key}", "Content-Type": "application/json", "User-Agent": "python-urllib/3.11"}
        )
...
class GeminiProvider(LLMProvider):
    ...
    def complete(self, prompt: str, system: str = None) -> str:
        ...
        req = urlrequest.Request(url, data=body, headers={"Content-Type": "application/json", "User-Agent": "python-urllib/3.11"})
        resp = urlrequest.urlopen(req, timeout=TIMEOUT_LLM)
        data = json.loads(resp.read().decode("utf-8"))
